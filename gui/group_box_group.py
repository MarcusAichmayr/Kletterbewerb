...
class GroupBoxGroup(QGroupBox):
    ...
    def setup_participant(self, participant: Participant) -> None:
        ...
        col = 0
        lbl = QLabel(self)
        lbl.setObjectName(f"lbl_participant_{self.name}_{col}")
        ...
        for r in self.routes:
            ...
            tries_layout.setObjectName(f"tries_layout_{participant.name}_{col}")
            spin_boxes = []
            for t in range(self.tries_per_route):
                spin_box = QSpinBox(self)
                spin_box.setObjectName(f"tries_spin_box_{participant.name}_{col}_{t}")
